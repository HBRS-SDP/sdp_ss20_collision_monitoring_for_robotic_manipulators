...
void Capsule::getShortestDirection(Eigen::Vector3d &shortestDirection, Box3 *box){
    ...
    // ROS_WARN_STREAM(shortestDirection.norm());
  ...
  std::vector<Line*>  Box3::EdgeList() const
    {
        ...
        // Line* arr[12];// = malloc(sizeof(char*)*12);
                ...
                for(int i = 0; i < 12; ++i)
            {
                ...
                // arr[i*2] = edge.basePoint;
		        // arr[i*2+1] = edge.endPoint;
            }
            return eArray;
    }
...
{
	 Eigen::Vector3d minPoint= this-> minPoint;
     Eigen::Vector3d maxPoint= this-> maxPoint;
	switch(cornerIndex)  
    { 
  		
		case 0: return minPoint;
		case 1: return Eigen::Vector3d(this->minPoint[0], this->minPoint[1],this-> maxPoint[2]);
		case 2: return Eigen::Vector3d(this->minPoint[0], this->maxPoint[1], this->minPoint[2]);
		case 3: return Eigen::Vector3d(this->minPoint[0], this->maxPoint[1], this->maxPoint[2]);
		case 4: return Eigen::Vector3d(this->maxPoint[0], this->minPoint[1], this->minPoint[2]);
		case 5: return Eigen::Vector3d(this->maxPoint[0], this->minPoint[1], this->maxPoint[2]);
		case 6: return Eigen::Vector3d(this->maxPoint[0], this->maxPoint[1], this->minPoint[2]);
...
/*  Deöete Eigen::Vector3d  Box3::OwnClosestPoint(Line *line)
{    
     Eigen::Vector3d l_pt1[8];
     Eigen::Vector3d l_pt2[6];
     Eigen::Vector3d sideCenters[6];
...
// Eigen::Vector3d Box3::ClosestPoint2(const Eigen::Vector3d &targetPoint)
// 		{
// 		  Eigen::Vector3d  point;	


//                     if (targetPoint[0]>maxPoint[0]){
//                             point[0]=   maxPoint[0]; 
//                     }

//                     else if (targetPoint[0]<minPoint[0]){
//                             point[0]=   minPoint[0]; 
//                     }
...
//                     if (targetPoint[1]>maxPoint[1]){
//                             point[1]=   maxPoint[1]; 
//                     }

//                     else if (targetPoint[1]<minPoint[1]){
//                             point[1]=   minPoint[1]; 
//                     }
...
//                     if (targetPoint[2]>maxPoint[2]){
//                             point[2]=   maxPoint[2]; 
//                     }

//                     else if (targetPoint[2]<minPoint[2]){
//                             point[2]=   minPoint[2]; 
//                     }
